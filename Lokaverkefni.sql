----Aftasta númerið í kennitölunni er til að skilgreina hvort manneskjan sé karlkyns eða kvenkyns, 0 = kvenkyns, 1 = karlkyns
...
as
begin
declare @heimilisfong as streets

declare gatacursor cursor
for
	select id, postnumer, heiti_nefnifall, heiti_thagufall 
	from @heimilisfong

declare @str varchar(75)
declare @str2 varchar(75)
declare @i int
declare @i2 int

open gatacursor
fetch next from gatacursor into @str, @i2, @str2
while @@FETCH_STATUS = 0
begin
	insert into gotur values(@str,@i2,@str2)
	fetch next from gatacursor into @str, @i2, @str2
end
close gatacursor
end*/
...
set @file = 'N:\2011\GSF\test.txt'
SET nocount ON 

CREATE TABLE #tempfile (line varchar(8000))
EXEC (INSERT #tempfile FROM "' + @file + '")
--insert into #tempfile values (@file)
...
exec sp_readTextFile 'C:\Users\TS295750\Desktop\text.txt'
*/
...
insert into @stelpur values('Ingi',511)
insert into @stelpur values('Vilhjálmur',500)
...
/*
insert into @stelpur values('Anna María',352)
insert into @stelpur values('Anna Margrét',209)
insert into @stelpur values('Anna Kristín',207)
insert into @stelpur values('Linda Björk',163)
insert into @stelpur values('Anna Sigríður',154)
insert into @stelpur values('Guðrún Helga',115)
insert into @stelpur values('Anna Guðrún',108)
insert into @stelpur values('Anna Lilja',105)
insert into @stelpur values('Eva María',98)
insert into @stelpur values('Kristín Helga',95)
insert into @stelpur values('Guðrún Sigríður',89)
insert into @stelpur values('Guðrún Ósk',87)
insert into @stelpur values('Helga Kristín',83)
insert into @stelpur values('Guðrún María',80)
insert into @stelpur values('Helga Guðrún',80)
insert into @stelpur values('Helga María',80)
insert into @stelpur values('Guðrún Jóna',77)
insert into @stelpur values('Íris Ósk',76)
insert into @stelpur values('Guðrún Margrét',75)
insert into @stelpur values('Berglind Ósk',74)
insert into @stelpur values('Eva Dögg',74)*/
...
select @maxid = MAX(id) from nafnogkt
select @maxid = @maxid + 1
exec randomgata @maxid
deallocate strakar
deallocate stelpur
...
-------------------------------------------------------------------------------------
exec StrakaNofn --insertar strákafornöfn í töfluna strakaFornofn með tíðni
exec StelpuNofn
exec nofn 1, 1, 17, 67 -- Fjöldi stráka, fjöldi stelpna, minnsti aldur, mesti aldur

delete from stelpurFornofn
delete from strakaFornofn
exec eydaurnafnogkt
exec finnanafn '%Elvar%'

select * from nafnogkt where heimilisfang like 'Grenimel%'
